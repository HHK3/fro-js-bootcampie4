...
function instruction10() {
  ...
  var result =
    "De string waar de bewerkingen op gedaan worden is: " + source + "<br/>";
  result += "De lengte van de string is: " + source.length + "<br/>";
  source = " " + source.substr(1);
  result +=
    "De eerste letter is vervangen door een spatie: " + source + "<br/>";
  result += "De lengte van de string is nog steeds: " + source.length + "<br/>";
  source = source.trim();
  result +=
    "Alle spaties aan het begin en eind zijn nu verwijderd: " +
    source +
    "<br/>";
...
var part4 = {
  ...
  instruction1: function() {
    ...
    var description =
      "Natuurlijk kun je alles opslaan in een string variable maar dat is niet handig. " +
    ...
    var properties = [
      "name = Yosef East",
      "group = MD2",
      "lessons = FRO,BAP en PROJ"
    ];
    description += getBulletListFromArray(properties);
    description += addLine(
      "Vervolgens ga je dit object gebruiken en stelt hiermee een string samen die er als volgt uitziet:</br>"
    );
    description += addLine(
      "Student Yosef East zit in groep MD2 en zijn eerste les is FRO"
    );
  ...
  instruction2: function() {
    ...
    var description =
      "<p>" +
      "JSON is een formaat dat heel veel gebruikt wordt in de wereld van webontwikkeling. " +
      "Met name bij het gebruik van een webAPI. " +
      "Dit betekent dat je vaak, en zeker wanneer je zelf web-API's gaat maken, " +
      "data moet omzetten (converteren) naar JSON." +
      "</p>";
    ...
    description +=
      '[{"name":"Yosef East","group":"MD2A"}, {"name":"Madison Dillard","group":"MD2A"}, {"name":"Thiago Sherman","group":"MD2A"}, {"name":"Iga Mcgee","group":"MD2A"}, {"name":"Amar Anthony","group":"MD2A"}, {"name":"Ilyas Suarez","group":"MD2B"}, {"name":"Jamel Vu","group":"MD2B"}, {"name":"Maja Garcia","group":"MD2B"}, {"name":"Elyas Galloway","group":"MD2B"}, {"name":"Duncan Bull","group":"MD2B"}]';
    ...
    for (var i = 1; i < studentsArray.length; i++) {
      var split = studentsArray[i].split(",");
      var result1 = {
        name: split[0],
        group: split[1]
      };
      ...
      if (i <= 9) {
        result += '{"name":"' + split[0] + '","group":"' + split[1] + '"}, ';
      } else if (i == 10) {
        result += '{"name":"' + split[0] + '","group":"' + split[1] + '"}';
      }
    }
  ...
  instruction3: function() {
    ...
    var description =
      "<p>" +
      "Er zijn veel publieke webAPI's beschikbaar waar iedereen gebruik van kan maken. " +
      "Voorbeelden hiervan zijn: postcodes, hoofdsteden, opkomst van de zon, etc.... " +
      "Jij gaat de namen ophalen van de astronauten die nu in de ruimte zijn en maakt daar een net lijstje van." +
      "</p>";
    ...
    xhttp.onreadystatechange = function() {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        result += "</ul>";
        updateContent(
          "Opdracht 14 - Publieke WebAPI gebruiken",
          description,
          result
        );
      }
  ...
  instruction4: function() {
    // Omschrijving van de opdracht
    var description =
      "Hoe leuk is het nu om een webAPI aan te roepen die jezelf gemaakt hebt. " +
      "Voor jouw heb ik er nu één gemaakt:" +
      "<p>" +
      "http://gert-rikkers.nl/api/students" +
      "</p>" +
    ...
    xhttp.onreadystatechange = function() {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        for (var i = 0; i < dataJSON2.data.length; i++) {
          if (i < 6) {
            result += "<li>" + dataJSON2.data[i].name + "</li>";
          }

          // Om alle studenten in het mooie lijstje te zetten
          // result += "<li>" + dataJSON2.data[i].name + "</li>";
        }
